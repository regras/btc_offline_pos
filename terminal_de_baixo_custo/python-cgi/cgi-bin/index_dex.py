...
def printContent():
	print '<div class="row">'
	print "<h1 class=\"productTitle\"><b>Á À á à ãBem-vindo a barraca de new-dog!</b></h1>"
	print '</div>'
	print '<p><small><h1><center>Escolha o teu produto!</center></h1></small></p>'
	...
	print '<p><h3><center>Preco unitario: 0.01 BTC</center></h3></p>'
	...
	print '<p><h3><center>Preco unitario: 0.02 BTC</center></h3></p>'
	###########################
	print '<center><form class="form-inline">'
	...
	print '<div class="caption">'
	print '<h2><center>Pamonha</center></h2>'
	print '<p><h3><center>Preco unitario: 0.01 BTC</center></h3></p>'
	############################
	print '<center><form class="form-inline">'
...
def printAdvertisement():
	...
	print '<div><h3 class="productAdvertise">Voce precisa pagar no total: <p class="label label-info" id="totalCost"></p></h3></div>'
	print '<h3 class="productAdvertise"><b>Clique na photo de produtos ou copia o endereco a seguir:</b></h3>'
	#print <p class="alert alert-info">Aviso: Copiar o endereco de bitcoin seguinte e o valor da transacao para o seu Wallet para construir a transacao</p></div>'
...
def responseForm():
	print '<div class="alert alert-danger"><h2><b>Copiar a transacao construida do seu wallet e colar aqui: </b></h2>'
	print '<form action="" method="post">'
	print '<div class="form-group">'
	print '<textarea class="form-control" rows="5" name="transactiondata"></textarea><br></div>'
	print '<button type="submit" class="btn btn-default" value="Submit">Enviar a transacao!</button></form></div>'
...
def admLoginPart():
	...
	print '<label for="loginstatement">Entra aqui a sua senha administrativa:</label>'
...
def admSetNewPassForm():
	print '<div class="alert alert-info"><h3><b>Mudar a senha: </b></h3>'
	print '<form action="" method="post" class="form-horizontal">'
	print '<div class="form-group">'
	print '<label class="col-xs-4 col-md-4 control-label">Digite a senha nova:</label>'
	print '<div class="col-xs-8 col-md-8"><input type="password" class="form-control" name="newPass"></div>'
	print '</div>'
	print '<div class="form-group">'
	print '<label class="col-xs-4 col-md-4 control-label">Repete a senha nova:</label>'
	...
	print '<button type="submit" class="btn btn-default">Mudar Senha</button></div>'
	print '</div>'
...
def admSeeTransaction():
	print '<div class="alert alert-danger"><h3><b>Fazer upload das transacoes nao carregadas:</b></h3>'
	print '<form action="" method="post">'
	...
	print '<tr>'
	print '<th>Id</th>'
	print '<th>Tempo Adicionado</th>'
	print '<th>Valor da transacao</th>'
	print '<th>Transacao Tx</th>'
	print '</tr>'
	#content
	with con:
		cur = con.cursor()
	...
	print '<button type="submit" class="btn btn-default">Fazer upload das transacoes selecionadas</button>'
	print '</form>'
	#botao para submeter todas as transacoes
	print '<form action="" method="post"><input type="hidden" name="uploadAll" value="uploadAll"><button type="submit" class="btn btn-default" value="Submit">Fazer upload de todas as transacoes</button></form>'
	print '</div>'
	#botao para expandir transacoes carregadas
	print '<button class="btn btn-default" id="seetrans">Mostrar as transacoes carregadas</button>'
	print '<p></p>'
	#info das transacoes carregadas
	print '<div id="uploaded" style="display: none;" >'
	#search uploaded transactions
	print '<div class="alert alert-danger"><h3><b>Deletar as transacoes carregadas:</b></h3>'
	print '''<form onsubmit="return confirm('Voce tem a certeza de deletar as transacoes?')" action="" method="post">'''
	#table with selectble entries
	print '<table class="table">'
	#header
	print '<tr>'
	...
	print '<button type="submit" class="btn btn-default">Deletar as transacoes selecionadas</button>'
	print '</form>'
	print '</div>'
	print '</div>'
...
if(admin != None):
	...
	if (checkPassword(passa) == "passed"):
		...
		print '<form action="" method="post"><input type="hidden" name="sendPage" value="sendPage"><button type="submit" class="btn btn-default" value="Submit">Proximo passo</button></form>'
		#responseForm()
		#admLoginPart()
	...
	print '<h1>Informacao das transacoes</h1>'
	with con:
		cur = con.cursor()
		uploaded = ('TRUE',)
		cur.execute("SELECT * FROM Transaction_Info WHERE verified =?", uploaded)
		...
		for row in rows:
			...
			if form.getvalue(transaction_info):
				try:
					...
					print "<p>A transacao %s foi deletada com sucesso</p>" % (sha_f)
						###################################################
					...
					print '<div class="alert alert-warning"><h3>Algum erro ocorreu e nao conseguiu deletar</h3></div>'
					break
	print '</div>'
	...
	with con:
		...
		for row in rows:
			...
			try:
				...
				print "<p>A transacao %s foi submetida com sucesso</p>" % (sha_f)
					###################################################
					########Add code for change transactions status####
					###################################################
					...
					#internet connection error
				print '<div class="alert alert-warning"><h3>Erro de conexao, por favor, verifique a conexao com Internet</h3></div>'
				break
	print '</div>'
	print '<form action="" method="get"><button type="submit" class="btn btn-default" value="Submit">Voltar</button></form>'
	...
	if newPassword != confirmNewPassword:
		print '<div class="jumbotron">'
		print '<h1>Por favor, entra senhas iguais, tente de novo!</h1>'
		print '</div>'
		print '<form action="" method="get"><button type="submit" class="btn btn-default" value="Submit">Voltar</button></form>'
	else:
		...
		passwd_file.close()
		print '<div class="jumbotron">'
		print '<h1>A senha foi modificada com sucesso!</h1>'
		print '</div>'
		print '<form action="" method="get"><button type="submit" class="btn btn-default" value="Submit">Voltar</button></form>'
	...
	print "<div><h3 class=\"productAdvertise\">Abra o teu wallet, cole o endereco <br><br><mark>%s BTC</mark> <br><br> e o valor a ser pago <br><br><mark>%s BTC</mark> <br><br> para produzir a transacao no teu wallet e cole esta transacao no campo abaixo:</h3></div> " % (bitcoinAddress,hiddenValor)
	print '<div class="alert alert-danger"><h2><b>Copiar a transacao construida do seu wallet e colar aqui: </b></h2>'
	print '<form action="" method="post">'
	print '<div class="form-group">'
	print '<textarea class="form-control" rows="5" name="transactiondata"></textarea><br></div>'
	...
	printAdvertisement()
	printBitcoinAddress(bitcoinAddress)
	print '<form action="" method="post"><input type="hidden" name="hiddenCost" id="hiddenCost" value=""><button type="submit" class="btn btn-default" value="Submit">Avancar</button></form>'
	...
	if checkTransaction(transactionText):
		...
		if len(data)==0:
			print '<div class="jumbotron">'
			print '<h1>Transacao aceita</h1>'
			print '<p>Foi feita uma transacao de: '
			print value_btc
			print 'BTC</p>'
			...
			print '<div class="jumbotron">'
			print '<h1>Transacao nao aceita</h1>'
			print '<p>Por favor, nao entra transacao duas vezes'
			print '</p>'
			print '</div>'
	else:
		#print alert message
		print '<div class="jumbotron">'
		print '<h1>Transacao nao aceita</h1>'
		print '<p>O dado de transacao esta incorreto'
		print '</p>'
		print '</div>'
	#print back button
	print '<form action="" method="get"><button type="submit" class="btn btn-default" value="Submit">Voltar</button></form>'
#print "<h2 class=\"productTitle\">Pagina de venda</h2>"
print '</div>'
print '</body></html>'
